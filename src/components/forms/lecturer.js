...
export default function LecturerForm({ currentLecturer, admin }) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: currentLecturer
      ? admin
        ...
        : api.updateLecturerByOrganizer
      : admin
      ? api.addLecturerByAdmin
      : api.addLecturerByOrganizer,
    retry: false,
    onSuccess: (data) => {
      toast.success(data.message || 'Sėkmingai išsaugota!');
      router.push(admin ? '/admin/lecturers' : '/dashboard/lecturers');
    },
    onError: (error) => {

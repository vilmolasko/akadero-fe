...
const formatDuration = (minutes) => {
  if (!minutes || minutes === 0) return '0 minučių';

  const hours = Math.floor(minutes / 60);
  const mins = minutes % 60;

  if (hours === 0) return `${minutes} minut${minutes === 1 ? 'ė' : 'ės'}`;
  if (mins === 0) return `${hours} valand${hours === 1 ? 'a' : 'os'}`;

  return `${hours} valand${hours === 1 ? 'a' : 'os'} ${mins} minut${
    mins === 1 ? 'ė' : 'ės'
  }`;
};
...
export default function CourseForm({
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: currentCourse ? api.updateCourseByAdmin : api.addCourseByAdmin,
    retry: false,
    onSuccess: (data) => {
      ...
      <div className='grid grid-cols-1 md:grid-cols-12 gap-6'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => formik.setFieldValue('organizer', v)}>
                <SelectTrigger>
        ...
        <Card className='col-span-12 md:col-span-4 space-y-4 p-5'>
          ...
          <div>
            ...
            {formik.touched.compensated && formik.errors.compensated && (
              <p className='text-red-500 text-sm mt-1'>
                {formik.errors.compensated}
              </p>
            )}
          </div>
